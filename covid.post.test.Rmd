...
So, we have an equation that seems to allow us to find the post-test probability given commonly estimated test operating characteristics.  Originally, I jumped to looking for studies to help me fill in the pieces of the second to last line of the display above [Note that in the last line, I rename things.  This renaming, interestingly, can cause some difficulty in interpretation; we understand the terms in the second to last line, but the renaming in the last line renames and strips the terms of their probabilistic meaning (along with removing information about what is being conditioned upon), which can lead to misuse].  It is worth noting however that test interpretation would be much better if there were databases kept such that we could just estimate $p(dz+|test-,sx, \text{ other variables})$ directly rather than having to use this equation.  This is completely possible, and would be much better and easier for the user of the test (I discuss this more below in what I call the "ideal" scenario).  This type  of modeling does not occur usually because each model would be specific to time and location, at least.  This sounds like a daunting task, therefore, to fit all of these models, but the reality is that if this is not done that way, it has to be done another way --- it just shifts the responsibility of interpretation from the test manufacturer or some independent organization onto the person using the test.  So, we have no choice - let's try to do this.
...
Now consider numbers from the recent meta-analysis [@bruemmer2021accuracy]. Note that these estimates, as far as I know, condition on nothing except the test result [and adult status?] (I do not know how they would otherwise pool so many estimates).  Hence, we have pre-test as 
$$
p(dz+)
$$
...
It seems that the asymptomatic just override the symptomatic (makes sense, fewer symptomatic.)  If this is true, it is important.  It means that the difference between the conditional and marginal estimates will be asymmetric.  I think this is common sense, going back to $\alpha$ above, which combines the two conditional distributions. 
...
It should (I think) be more likely that a symptom is negative given that a test is negative.  There is no reason to believe, we see now, that the marginal $p(dz+|test-)$ will be halfway between $p(dz+|test-,sx+)$ and $p(dz+|test-,sx-)$.  Hence,  if you are asymptomatic, you will get a decent (conditional) estimate;  if you are symptomatic, you will be pulled toward asymptomatic, and get a poor (conditional) estimate.

I am going to explore this a bit more. We will make our variables $Z\sim Bern(expit(\beta X)).$
```{r }
...
This is contrary to my expectation that we would interpolate between the two conditionals with weight placed according to $p(sx|test-)$.
