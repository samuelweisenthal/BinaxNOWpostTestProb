...
To begin this document, I will consider the easier problem of estimating post-test probability given a negative test only, with no exposure.  In other words, let us forget about the conditional post-test probability,  $p(dz+|test-,exposure),$ and consider the "$p(dz+|test-)$" (this is almost always done, unfortunately, when we talk about post-test probabilities).  

## The difficulty of pre-test probability $p(dz+)$

Before doing this, however, let us consider an even simpler problem: $p(dz+).$  We will need to have some sense of this in order to estimate $p(dz+|test-)$ (we need this in order to make this estimate with the information I have available; we do not need to estimate $p(dz+)$ if we had other information that I do not have available).  $P(dz+)$ is what is commonly called the "pretest" probability.  Often, it is considered to be prevalence.  However, this is a very nebulous estimand.  We almost never think of a pure, unconditional probability [@moons2003sensitivity].  It, however, is also somewhat constrained.  I show the derivation just below partly to show how $p(dz+)$ must align with $p(test-|dz+).$ Often, $p(test-|dz+)$ is conditional on things that are not mentioned explicitly.  Hence, we need to estimate $p(dz+)$ conditional on covariates that were possibly not even reported in the study that evaluated the test.  Hence, estimating $p(dz+)$ is quite difficult, and the estimate cannot be just "what you expect your pretest probability is." It also cannot be your best guess at your probability of the disease given your current characterstics, such as symptoms, location, etc. It cannot be estimated based on more or less information than was collected in the study that evaluated the test operating characteristics. 

In some ways, it may be best to stop here. However, I needed an answer at the time, and it is likely that many need an answer now.  So, we will proceed with what we have. Before, though, I will give a brief summary now of what I believe would be the ideal way to solve the test interpretation problem.  Then, because this is not available to us, and we still are looking for an answer, I will proceed forward based on what is available.  The approach that I will take, using Bayes rule, is not ideal, but it is the standard approach in the literature (see, for example, on the general use of Bayes rule [@akobeng2007understanding]; more specifically, for covid-19, see [@good2020interpreting; @watson2020interpreting; @stites2020interpretation]).

# Ideal test interpretation framework

Note that to obtain the graphs above, we are working with what we have available. This all gets somewhat confusing.  One way to remove some (not all) difficulty here is to just take a direct approach from the beginning, which I will call the "ideal" approach.  However, the following would be ideal: every time one takes a test, one goes to a website and enters information such as age, sex, zip code, and test result. The website then calculates post-test probability based on a model for, explicitly,
...
Now, let us consider the problem of estimating  $p(dz+|test-)$.  We  have, by Bayes rule,
...
We start with the likelihood ratio.
...
Aside: will there be issues with the observational nature of the data? note that, just focusing on $p(dz+|test,sx),$  we are really interested in the test effect. Let us therefore consider how this would be estimated in general.  First, if the data is observational and there is an unmeasured variable that affects whether someone takes a test and also whether they have the disease (confounding), the effect estimate may be incorrect. Generally, we would condition on the confounder to remove this issue. I am still trying to think more about how this relates to the discussion below on conditioning on the necessary set of variables. I think it is different, since we can defend $p(dz+|test)$, if it is tailored enough to our covariates to inform decision-making, but we can't really defend an unadjusted estimate of, e.g., a treatment effect.  This actually seems to be the tension here.  In some cases, $p(dz+|test-)$, $p(dz+|test-)$,  is good enough, even though ideally we would have conditional $p(dz+|test-,age,sex,location,time,incubation,\dots)$.  Especially, $p(dz+|test-)$ is good enough if somehow the variables we do not condition on in the $p(dz+|test-)$ estimate sort of cancel each other out.
...
## Weakening the assumption of negligibility (aka, that the $p(dz+|test-)$ estimate is fine):
...
Let us look at whether symptom status is reasonable to $p(dz+|test-)$ize.
...
## [@prince2021evaluation] vs  [@bruemmer2021accuracy]
...
It seems that the difference between the $p(dz+|test-)$ and $p(dz+|test-)$ estimates will be asymmetric.  We expected that the $p(dz+|test-)$ would be between the two conditionals; going back to $\alpha$ above, which combines the two conditional distributions, we see. 
...
plot.p(pts,
       post.label="p(dz+|test-,...)",xlab="Pre-test: p(dz+|...)",
       leg.labels=c(TeX("p(dz+|test-,sx+)")," p(dz+|test-)",TeX("p(dz+|test-,sx-)")),main="p(dz+|test-,sx+) vs. p(dz+|test-) (simulation)"
       )
...
#pts$$p(dz+|test-)$$m)
```
Again, this is contrary to my expectation.  I will have to think more about this.  I am pretty sure the following probability statement, copied from above, is not wrong...
...
In any case, it should be certain that the $p(dz+|test-)$ falls within the conditionals.  It should be certain that the $p(dz+|test-)$ might be closer to one than to another. It is therefore the case that how much you will lose out by constraining yourself to the study covariates depends on your covariates.   
