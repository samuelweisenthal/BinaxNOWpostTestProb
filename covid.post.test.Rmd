...
To begin this document, I will consider the easier problem of estimating post-test probability given a negative test only, with no exposure.  In other words, let us forget about the conditional post-test probability,  $p(dz+|test-,exposure),$ and consider the "$p(dz+|test-)$".  
...
I am going to show my code to promote transparency and reproducibility.  [You can find the notebook in my GitHub. You are free to use and modify this code as long as you allow others to use and modify your modification of this code.  Use this code at your own risk, and I will not be responsible for maintenance.]
...
points(.2,pdzptsnsxp,col=colorBlindBlack8[1],pch=19)
points(pdzpsxp,pdzptsnsxp,col=colorBlindBlack8[1],pch=19)
points(.2,pdzptsn,col=colorBlindBlack8[2],pch=19)
points(.2,pdzptsnsxn,col=colorBlindBlack8[3],pch=19)
...
This corresponds to what we see with  $p(dz+|test-,sx)$ from [@prince2021evaluation] with $p(dz+|test-)$  [@bruemmer2021accuracy]. Both are contrary to my expectation.  I will have to think more about this.  I am pretty sure the following probability statement, copied from above, is not wrong...

$$
p(dz+|test-) = p(dz+|test-,sx+)p(sx+|test-)+p(dz+|test-,sx-)p(sx-|test-)
$$
Doesn't this imply that the $p(dz+|test-)$  should be closer to $p(dz+|test-,sx-),$ since $p(sz-|test-)$ is larger? 
