...
## Obtaining estimates of the necessary quantities

The next question is whether we can obtain estimates of the desired quantities from [@prince2021evaluation].  This depends on the study design.  A retrospective case-control is most appropriate [@CM], but why?

Note that there are two ways to go about interpretation is that we define the outcome as disease status and then we check whether there was a test result. This would be the case if $p(dz+|test-,sx)$ were our target. Or is it the other way around; we define the outcome as test result, as would be the case with $P(test-|dz+,sx)$.  Based on Bayes rule, you can technically use either one of these, although the latter causes more difficulty. The study does provide an estimate of $P(dz-|test-,sx)$, so we can obtain $P(dz+|test-,sx)=1-P(dz-|test-,sx).$

Note that, just focusing on $P(dz+|test,sx),$  we are really interested in the test effect. Let us therefore consider how this would be estimated in general.  First, if the data is observational and there is an unmeasured variable that affects whether someone takes a test and also whether they have the disease (confounding), the effect estimate may be incorrect.  I am trying to think more about how this relates to the discussion below on conditioning on the necessary set of variables, since we typically deal with confounders by conditioning on them. 



Assuming that [@prince2021evaluation] was conducted in such a way that the estimation of these quantities is valid, we have from their study point estimates and 95% confidence intervals for
...
Hence, the probability of $P(dz+|test-,x)$ that we computed above is somewhere in between what we really want to know, $P(dz+|test-,x,exp+),$ and the $P(dz+|test-,x,exp-),$ which is the probability under no sustained exposure. This may appear to not be a big deal. However, consider now what happens in a similar situation 

$$
p(dz+|test-) = (dz+|test-,sx+)P(sx+|test-)+P(dz+|test-,sx+)P(sx+|test-)
$$

Based on [@prince2021evaluation] (and as I will show below graphically), the presence of symptoms matters.  Likewise, we have no reason to believe that the presence of exposure does not matter.  This might go the other way; we may say, ``well, symptoms seems to make a difference, but having the marginal estimate $p(dz+|test-)$ (somewhere in between) would not really change my behavior.'' Further, we may say, ``Exposure matters, but it is unlikely that it is as important as e.g. symptoms, so we can expect the information loss we get by marginalizing over exposure to be even less than the information loss we get from marginalizing over symptoms, and therefore we might as well proceed with $P(dz+|test-)$ rather than $P(dz+|test-,exposure+).$'' 
